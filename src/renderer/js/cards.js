...
function renderCard(card, level) {
  ...
  document.getElementById('challenge-points').textContent = `${card.points} PTS`;
  document.getElementById('challenge-meta').textContent = `NIVEL ${level} · ${card.color.toUpperCase()}`;
  document.getElementById('challenge-question').textContent = card.question;

  const challengeBadge = document.getElementById('challenge-badge');
  challengeBadge.className = `card-badge ${colorClass}`;
